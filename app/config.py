...
class DaytonaSettings(BaseModel):
    daytona_api_key: Optional[str] = Field(default="", description="Daytona API key")
...
class Config:
    ...
    def _load_initial_config(self):
        ...
        # 支持从环境变量读取 API Key（如果配置中是占位符或为空）
        api_key = base_llm.get("api_key", "")
        if api_key in ["YOUR_ARK_API_KEY", "YOUR_API_KEY", ""] or api_key.startswith(
            "YOUR_"
        ):
            import os

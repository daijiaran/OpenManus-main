...
class StrReplaceEditor(BaseTool):
    ...
    async def _view_directory(path: PathLike, operator: FileOperator) -> CLIResult:
        ...
        # Use Python to list directory to avoid encoding issues with find command
        path_obj = Path(path)
        ...
        try:
            ...
            result_lines = [f"Here's the files and directories up to 2 levels deep in {path}, excluding hidden items:\n"]
            ...
            try:
                ...
                for item in sorted(path_obj.iterdir()):
                    ...
                    # Level 2 (children of directories only)
                    if item.is_dir():
                        try:
                            for subitem in sorted(item.iterdir()):
                                if subitem.name.startswith('.'):
                                    continue
                                result_lines.append(f"  {subitem.relative_to(path_obj)}")
                        except (PermissionError, OSError, UnicodeDecodeError) as e:
                            result_lines.append(f"  [Error reading subdirectory: {str(e)}]")
            ...
            # Fallback to find command if Python listing fails
            import sys
            if sys.platform == "win32":
                # Windows: use dir command
                find_cmd = f'dir /s /b /a-d "{path}" 2>nul'
            else:
                find_cmd = f'find "{path}" -maxdepth 2 -not -path "*/.*" 2>/dev/null'
